...
#### 4.6.1) Variables и `terraform.tfvars`
...
У `infra/aws/odoo-dev-ec2-compose/` и `infra/aws/odoo-prod-ec2-compose/` често ћеш видјети:
- фајл `variables.tf` (у `infra/aws/odoo-dev-ec2-compose/`) као што из назива фајла може да се закључи, садржи списак варијабли и њихових вриједности које stack очекује (која имена variables постоје, ког су типа и шта је опционо или има default вриједност); нпр. ту видиш да Terraform очекује `instance_type`, `domain_name` или `allowed_ssh_cidr`,
- фајл `terraform.tfvars.example` као примјер како те variables попуњаваш за конкретан environment: то је template који кандидат или оператор чита и по њему прави свој стварни конфиг,
- фајл `terraform.tfvars` као стварне, локалне вриједности које Terraform користи при `plan`/`apply`: ту заиста стоји шта је за тај clone и тај environment `instance_type`, који је домен, који је SSH CIDR и слично; у PROD контексту то је предвиђено локално мјесто `infra/aws/odoo-prod-ec2-compose/terraform.tfvars`, ако га оператор заиста креира за тај environment.

Важно: DEV и PROD stack овдје намјерно не користе исти secret model. Зато се за PROD стварни `infra/aws/odoo-prod-ec2-compose/terraform.tfvars` у репоу не version-ује, а чак и кад га оператор локално направи, он не смије садржати PROD тајне. По том моделу, тајни подаци се чувају на самом PROD EC2 серверу, а не у Terraform git артефактима; то је дефинисано у `infra/aws/CODEX_TERRAFORM.md`, поглавље `5.2 Terraform apply`.
...
Практично: кад Terraform подигне EC2, није довољно да имаш само машину; `network.tf` објашњава у ком мрежном простору је та машина, како добија public route и каква је њена basic network позиција.
...
## 6) Шта Terraform код често значи у овом репоу
...
## 10) Minimal safe workflow у KomITi-ју
...
## 12) Foundations practical drill
...
## 13) Local Terraform + Docker Desktop lab за кандидата

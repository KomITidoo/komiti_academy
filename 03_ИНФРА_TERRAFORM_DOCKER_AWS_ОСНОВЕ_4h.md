...
### 4.5) Variables и `terraform.tfvars`
...
- фајл `terraform.tfvars` као локалну, некомитовану runtime конфигурацију, нпр. у `infra/aws/odoo-dev-ec2-compose/terraform.tfvars`; за PROD је то предвиђено локално мјесто `infra/aws/odoo-prod-ec2-compose/terraform.tfvars`, ако га оператор заиста креира за тај environment.

Важно: зато не видиш `infra/aws/odoo-prod-ec2-compose/terraform.tfvars` у репоу. За PROD је намјерно versioned само `terraform.tfvars.example`, а стварни `terraform.tfvars` оператор по потреби креира у свом локалном clone-у репоа `odoo4komiti`, у директоријуму `infra/aws/odoo-prod-ec2-compose/`, из тог template-а, или осјетљиве вриједности уноси преко environment variable-а. То значи да је та путања предвиђено локално мјесто за фајл, али није гаранција да фајл већ постоји на сваком лаптопу или сваком clone-у. Ако га не видиш у свом clone-у, то не значи да се „изгубио“ из git-а, него најчешће само да још није локално materialized за тог конкретног оператора и ту конкретну машину. Дакле, „локално“ овдје значи на операторској машини или контролисаном runner-у, а не на самом EC2 host-у. Разлог је једноставан: PROD `tfvars` носи environment-specific и често secret вриједности, па не смије бити нормалан git артефакт.
...
### 4.6) Структура директоријума: општа и KomITi конкретна
